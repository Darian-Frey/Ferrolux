...
its own panel — nothing in the window comes from a desktop theme. **311 checks
...
- `src/ui/ThemeTokens.{h,cpp}` — a named token set loaded from JSON
- `src/main.cpp` — entry point, all inter-module wiring, settings persistence
- `qml/` — fourteen components and `shaders/`. `Main.qml` is the window's
  layout; everything it draws with is panel chrome
...
**No open bugs and no suggested improvements.** Twenty bugs found so far,
nineteen fixed and one won't-fix upstream (BUG-006). Read BUGS.md before
