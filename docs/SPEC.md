...
| `meters/flame-back` | double | 1.15 | Height of the furthest |
| `meters/flame-parallax` | double | 2.2 | Sideways drift front to back, in band widths |
...
**The palette is eight tokens and stays eight.** A lit display uses more colours than that — a peak cap, a needle, the two ranks of the flame, an over-reference segment, an unlit one — and none of them is in the table, deliberately. Measured against `readout`, each is the same lamp at a different hue and lightness, which is what a single-phosphor display physically is; they are therefore *derived* in `qml/Tokens.qml` from the lamp rather than written down per set. A finish that changes the lamp gets them all without specifying any, and cannot end up disagreeing with itself about what colour its own display is. See IMP-006 for the fitted offsets and the measurements. A future finish that genuinely needs a second lamp colour — a ladder with real red over-segments — would be a palette addition and belongs here, argued on its own.
