...
- [~] Panel chrome, bezels and control surfaces as QML components — the chassis, `qml/PanelSection.qml` (a well or a raised surface, one boolean apart), `qml/PanelButton.qml` (a moulded face with travel), `qml/Slot.qml` (a continuous control that never writes its own value, per BUG-009), `qml/SlideSwitch.qml`, `qml/Legend.qml` and `qml/TransportGlyph.qml`. The playlist is a lit well and the transport and play-order controls are panel chrome; the equaliser, the file toolbar and the volume and balance controls still sit on Qt Quick Controls
...
- [~] Playlist and equaliser sections styled to match — the playlist is a well, with rows in the readout face at three brightnesses rather than in three hues, the playing entry marked as well as brightened, and a selected row backlit in the dimmest amber instead of the desktop's highlight colour. The equaliser is a raised surface with eleven faders, lit gain readouts over unlit ghosts and printed band centres. What remains on Qt Quick Controls is the two toolbars — the file commands, the sort and preset menus, and the bypass checkbox
