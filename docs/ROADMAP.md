...
**Status:** Feature-complete 2026-09-04; one acceptance clause is the author's to judge
**Features delivered:** F-040, F-041, F-042, F-044, F-036
**Deliverables:**
- [x] Design token set: palette, radii, bevel geometry, typography — `resources/themes/*.json` loaded by `ui/ThemeTokens`, with the vocabulary spelled once in `qml/Tokens.qml` and scaling applied there and nowhere else. The four faces are bundled under `resources/fonts/` and generated by `tools/make-fonts.sh`; `tests/tokens_test` holds every set to SPEC.md value by value, to one shared vocabulary, and to contrast floors, and asserts each face loads under the family the set asks for
- [x] Panel chrome, bezels and control surfaces as QML components — `qml/PanelSection.qml` (a well or a raised surface, one boolean apart), `qml/PanelButton.qml` (a moulded face with travel), `qml/Slot.qml` (a continuous control in either orientation that never writes its own value, per BUG-009), `qml/SlideSwitch.qml`, `qml/PanelMenu.qml`, `qml/EntryField.qml`, `qml/Readout.qml`, `qml/Legend.qml` and `qml/TransportGlyph.qml`. Nothing in the window is drawn from a Qt Quick Controls style any longer: what remains of Controls is `ApplicationWindow`, the native file choosers, one `Dialog`, a `ScrollBar` and `Popup` as the base of `PanelMenu` — all of them behaviour rather than appearance
...
- [x] Playlist and equaliser sections styled to match — the playlist is a well, with rows in the readout face at three brightnesses rather than in three hues, the playing entry marked as well as brightened, and a selected row backlit in the dimmest amber instead of the desktop's highlight colour. The equaliser is a raised surface with eleven faders, lit gain readouts over unlit ghosts and printed band centres. Both toolbars are moulded buttons with printed legends; the sort and preset lists are lit wells, the bypass is a switch, and the filter is a lit field with an unlit prompt rather than grey placeholder text
- [x] The transport state and track counter moved into the display, where they had always belonged: they are values the instrument reports, and they were on the chassis in the system font only because the harness put them there before there was a display to put them in
...
- [x] Compact mode (F-042) — the panel folds to a display-and-transport strip and back, with the window position unchanged and playback untouched. Nothing in the fold reaches the engine, so playback state is preserved by construction rather than saved and restored; the drawer keeps its own open state, so folding and unfolding returns the equaliser to how it was left. Persisted under `ui/compact`. Volume sits in the transport row rather than the mixing section, so it is present in the strip as well as in the full panel — one control in both modes rather than two bound to the same property
...
- [x] Four finishes and an invertible display (F-044), added during the phase at the author's request — `ferric`, `anodised`, `glacier` and `ember` are the same file with a different palette, so the geometry and the type scale are shared by construction rather than copied and kept in step, and they are exchanged while the panel is running. The variants change the *chassis* and keep the amber lamp, which is how real equipment varies. `ui/display-inverted` draws the display well lit-ground with dark text instead, which is a different instrument rather than the same one recoloured. `tests/tokens_test` holds every set to one vocabulary and to contrast floors, which is BUG-020 turned into a check

- [x] Adjustable display proportions (F-036), added after the phase at the author's request — nine values the meter shaders carried as literals become the user's, held by `src/ui/VisualSettings` and clamped in the setters so that neither a slider nor a hand-edited settings file can reach a division by zero inside a shader. They are settings rather than tokens, and therefore survive a change of finish. Set from `qml/SettingsWindow.qml`, a second top-level window rather than a panel drawer, because a proportion can only be judged by watching the display move while it changes — and the drawer covered the display being adjusted. Eleven keys in SPEC.md §Settings; nine checks in `tests/meters_test`
...
> **The first clause is met and measured**, 2026-09-04, by `tools/verify-scaling.sh`:
> one device pixel of edge rise at 1×, 1.5×, 2× and 3×, and each capture reduces
> back to the 1× panel rather than to a differently laid-out one. That is also
> AV-005's detection, outstanding since the documents were written.
>
> **The second clause cannot be measured and is not claimed.** Whether a viewer
> reads the panel as hardware is a judgement, and the person to make it is not
> the one who drew it. Every deliverable is done and every measurable criterion
> passes; this phase is marked feature-complete rather than complete until that
> judgement is made.

**Five specification faults found by drawing it.** SPEC.md's Handjet weight
rendered the dot-matrix face as continuous strokes, failing the intent stated
beside the value that set it, and the same investigation established a minimum
legible size for that face which binds the playlist as well as the title
(BUG-017). SPEC.md cited an OFL Reserved Font Name clause that Handjet does not
invoke (BUG-018). The lit-over-unlit readout arrangement needs a ground darker
than both its layers, so `display-bg` is a requirement rather than a label — put
a readout on the chassis and the unlit ghost out-contrasts the lit value.
The legend face carries none of the transport symbols, so they are drawn. And
the dot-matrix ghost codepoint left provisional here resolves to none: the
instance has no full-cell glyph.
