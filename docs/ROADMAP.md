...
- [x] Compact mode (F-042) — the panel folds to a display-and-transport strip and back, 780 to 175 to 780, with the window position unchanged and playback untouched. Nothing in the fold reaches the engine, so playback state is preserved by construction rather than saved and restored; the drawer keeps its own open state, so folding and unfolding returns the equaliser to how it was left. Persisted under `ui/compact`
