...
**`platform/`** contains everything that is about the desktop rather than about audio. Isolating it means the rest of the application has no direct dependency on D-Bus or on the session type, and it reaches the player through `app/Player` rather than through `core/` directly, so the coupling runs one way. `MprisService` is the D-Bus surface of F-050 and talks only to `app::Player`, so a lock screen and a button on the chassis go down the same path. `Settings` owns every key in SPEC.md §Settings, restores them onto the objects and writes them back on `aboutToQuit`, and takes the window as a plain `QObject` read by property name so that `platform/` needs no dependency on Qt Quick to remember how the panel was left. Where the file lands is `QStandardPaths`' answer rather than GStreamer's, which is why persistence belongs here and not in `core/`. `MediaKeys` is F-051, and it prefers registering with a desktop settings daemon to grabbing keys: the major desktops own the media keys themselves and do not route them through MPRIS, which is a split by desktop rather than by display server and is not what the feature note assumed. `X11MediaKeys` is the last resort for a session with neither — a global grab, reached only when no daemon answered, and held for the run rather than by activity because letting go of a grab gives the keys to nobody (IMP-008). It is behind `find_package(X11)`, so a Wayland-only or headless build compiles it to a stub. `SingleInstance` and `CommandLine` are F-052. The instance name is claimed before the pipeline is built, so a second launch hands its paths to the running player and exits without ever opening the audio device — two processes briefly holding the same sink is audible, and it would happen on every file opened from a file manager. `Session` is F-015: the playlist contents go to a playlist file, because that is what a playlist file is for and it stays readable by anything else, while the track, the position and the play *order* go to SPEC.md §Settings. The order is the permutation rather than the shuffle flag — F-012 requires it to be held rather than recomputed, and reshuffling on restore is that recomputation performed at launch. Still to come in Phase 6: the desktop entry. **Qt's D-Bus module is linked by the application target alone** — no test suite links it, which is what makes the isolation checkable rather than merely intended.

**`resources/`** holds the four OFL faces of D-012, the token sets, and the desktop entry with its icon. The icon is an SVG rather than a set of rendered sizes, which is the project's founding argument (D-003, F-041) applied to the one asset that lives outside the window; it is drawn flat, with no gradients, because an icon is rendered by whatever the desktop ships and renderers disagree about gradients in a way they do not about a filled rectangle.
...
**`qml/`** is presentation only, and is flat rather than divided into subdirectories: seventeen components and a `shaders/` directory of GLSL sources compiled by `qsb` at build time. The flat layout reflects how they are used — `PanelSection`, `PanelButton`, `Slot`, `SlideSwitch`, `Readout`, `Legend` and the rest are a vocabulary drawn on by every part of the panel, not a set of screens.
...
`Shortcuts.qml` is the one component with no appearance at all. It holds the keyboard table F-043 binds from, and `KeyGuide.qml` prints that same table — one declaration serving both, so the reference cannot describe a key that does nothing. `Slot.qml` is keyboard-operable for the same feature, and every keypress leaves through `moved` and `released` exactly as a drag does, so a caller's policy about when to act applies to both without the control knowing which it is handling.
